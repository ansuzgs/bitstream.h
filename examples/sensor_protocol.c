...
static void print_reading(int index, const sensor_reading_t *r) {
    printf("  [%d] device=%2u  bat=%s  temp=%+6.1f°C  hum=%3u%%  alert=%s\n", index, r->device_id,
           r->battery_low ? "LOW " : "ok  ", raw_to_temp(r->temp_raw), r->humidity,
           r->alert ? "YES" : "no");
}
...
int main(void) {
    ...
    sensor_reading_t readings[] = {
        {.device_id = 1,
         .battery_low = false,
         .temp_raw = temp_to_raw(22.5),
         .humidity = 45,
         .alert = false},
        {.device_id = 2,
         .battery_low = true,
         .temp_raw = temp_to_raw(-10.3),
         .humidity = 89,
         .alert = true},
        {.device_id = 15,
         .battery_low = false,
         .temp_raw = temp_to_raw(0.0),
         .humidity = 50,
         .alert = false},
        {.device_id = 7,
         .battery_low = false,
         .temp_raw = temp_to_raw(55.8),
         .humidity = 12,
         .alert = true},
    };
    ...
    for (int i = 0; i < count; i++) {
        ...
        /* Verify round-trip */
        if (r.device_id != readings[i].device_id || r.battery_low != readings[i].battery_low ||
            r.temp_raw != readings[i].temp_raw || r.humidity != readings[i].humidity ||
